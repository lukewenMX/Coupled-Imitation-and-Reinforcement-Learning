...
class Husky(object):
    ...
    def __init__(self):
        ...
        # self.position_sub = rospy.Subscriber('/odometry/filtered',Odometry,self.poseCallback)
    ...
    def discretize_observation(self,laser,new_ranges=10):
        ...
        for i, item in enumerate(laser.ranges):
            if (i % mod == 0):
                if laser.ranges[i] == float('Inf') or np.isinf(laser.ranges[i]):
    ...
    def get_state(self,goal):
        ...
        x = self.pose[0] - goal[0] # Since the distance was derived by substracting the target position from the current position.
        y = self.pose[1] - goal[1]
        related_pose = np.array([x,y])
        dis = np.linalg.norm(related_pose) # Get the distance between the current position and the desired target.

        # The following is aiming to get the respective angle for target position in the husky robot l0cal frame.
        targetAngle = 0.0 
        if( (related_pose[0] > 0) and (related_pose[1] > 0)):
            ...
            targetAngle = -180.0 + posAngle_degress # Due to the substracting of target position from the current position.
