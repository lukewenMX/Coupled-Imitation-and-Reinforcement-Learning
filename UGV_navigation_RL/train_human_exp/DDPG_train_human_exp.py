...
class Actor(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.Relu(x)
        x = self.forward3(x)
        x[:,0] = self.sigmoid(x[:,0]) #need to be modify!
        x[:,1] = self.tanh(x[:,1])
        return x
...
class DDPG(object):
    def __init__(self,
                 a_dim,
                 s_dim,
                 LR_A = 0.0005, # learning rate for actor 0.001
                 LR_C = 0.001,  # learning rate for critic 0.005
                 GAMMA = 0.99,  # reward discount  0.9S
                 TAU = 0.005,   # soft replacement  0.0001
    ...
    def store_transition(self, s, a, r, s_, done,kl):
        transition = np.hstack((s, a, [r], s_, [done],[kl]))
    ...
    def Learn(self):
        ...
        s_ = batch_memory[:, -self.s_dim - 2:-2].float()
        a = batch_memory[:, self.s_dim:self.s_dim + self.a_dim].float()
        r = batch_memory[:, self.s_dim + self.a_dim].view(-1,1).float()
        #d = batch_memory[:, -1]
        d = batch_memory[:, -2].view(-1,1).float()

        # ---------------------- optimize critic ----------------------
        # Use target actor exploitation policy here for loss evaluation
        a_ = self.actor_target(s_).detach()
        a_[:,0] = torch.clamp(a_[:,0],min=0,max=1.0).detach()
        a_[:,1] = torch.clamp(a_[:,1],min=-0.5,max=0.5).detach()
        ...
        self.optim_c.step()
        self.loss_critic_list.append(loss_critic.detach())
        ...
        self.optim_a.step()
        self.loss_actor_list.append(loss_actor.detach())
