...
def train(args):
    ...
    a1 = np.array([np.mean(memory_init[:,30:33],axis=0)]) # (1,3)
    b1 = np.cov(memory_init[:,30:33].T)
    ...
    for epoch in range(args.train_epoch):
        ...
        while True:
            ...
            if terminal or episode_step > args.train_step:
                break
        a2 = np.array([np.mean(np.array(online_experience)[:,30:33],axis=0)])
        # print(a2)
        b2 = np.cov(np.array(online_experience)[:,30:33].T)
        # print(b2)
        try:
            kl = KL(a1,b1,a2,b2,3)
        ...
        if kl < 2000:
            ...
            index = model.memory_counter % model.memory.shape[0]
            model.memory[index:index+len(online_experience),:] = online_experience
        ...
        if (epoch+1) % args.epoch_store == 0:
            ...
            # plt.clf()
            #plt.close()
        if len(total_reward_list) > 25:
            reward_list = np.array((total_reward_list))
            reward_true = reward_list[-25:] > 3.5
